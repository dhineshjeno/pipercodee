...
import { users, friendRequests } from '../data/mockData';
...
export const Friends: React.FC = () => {
  ...
  const friendActivities = activities.filter((activity) =>
    friends.some((friend) => friend.id === activity.userId)
  );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-indigo-500/20 text-indigo-400 rounded-lg border border-indigo-500/30 hover:bg-indigo-500/30 transition-all"
          >
          ...
          <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700/50 mb-6">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 bg-slate-900/50 border border-slate-700 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-3">
          {friends.map((friend) => (
            <Link
              ...
              className="block bg-slate-800/50 rounded-xl p-6 border border-slate-700/50 hover:border-slate-600/50 transition-all"
            >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {friend.currentActivity ? (
                    <div className="flex flex-wrap items-center gap-3">
                      <LanguageBadge
                        language={friend.currentActivity.language}
                        size="sm"
                      />
        ...
        <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700/50">
          ...
          <ActivityFeed activities={friendActivities} maxItems={5} />
        </div>
      </div>
    </div>
  );
